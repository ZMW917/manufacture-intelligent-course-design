...
输入 → 处理 → 输出
==================
输入契约（重要）：本模型接收的是「经 image_preprocess 预处理后的熔池 ROI 增强图」，
而非原始拍摄图像。训练与推理必须走同一条预处理链路，否则会出现训练/服务不一致
（表现为训练准确率很高、线上准确率接近随机）。

输入：
  - 训练：images_dir（原始熔池灰度图目录） + labels_csv（含 sample_id / quality_label 两列）
  - 预测：单张熔池图像（H×W 或 H×W×3 的 numpy 数组，或图片文件路径），
          内部按与训练相同的流程先做 ROI 提取与增强

处理：
  1) 读入原始图后先调用 image_preprocess.preprocess_image 提取熔池 ROI 并做直方图
     均衡增强，得到与线上推理完全一致的输入
  2) 顶层模型 = ResNet-18（torchvision.models.resnet18(weights=None)），离线随机初始化、
     不下载预训练权重；把第一层 conv1 改为 1 通道，最后一层 fc 改为 num_classes 输出
  3) 输入统一 resize 到 64×64 单通道、像素归一化到 [0,1]，再按均值/方差 0.5/0.5
     标准化到 [-1,1]（简化处理：不额外统计训练集均值方差，训练/预测共用同一常数）
  4) 训练：固定随机种子(42)，按 80/20 划分 train/val，CrossEntropyLoss + Adam，
     每个 epoch 打印 loss / train_acc / val_acc，并保存验证集最优的模型权重
  5) 推理：与训练一致的预处理后前向传播，softmax 得到三分类概率
...
class CNNClassifier:
    ...
    def predict(self, image):
        ...
        入参是**原始**熔池图像；函数内部会执行与训练一致的 ROI 提取与增强，
        调用方无需（也不应）自行预处理，否则会与训练链路不一致。
        ...
        if isinstance(image, (str, os.PathLike)):
            ...
            raw = _to_gray_uint8(image)

        # 与训练完全一致的预处理：ROI 提取 + 直方图均衡增强
        gray = preprocess_image(raw)["enhanced"]
