...
```
├── README.md            # 项目说明（本文件）
├── 设计说明书.md        # 设计说明书（背景调研/方案设计/数据集构建/技术方向/界面/调试/总结展望）
├── 选题说明.md          # 选题说明
├── 方案设计.md          # 方案设计
├── 学习笔记.md          # 学习笔记
├── 选题汇总.md          # 61 个同学选题汇总（选题调研产物）
├── 一键启动.bat         # 一键启动脚本（自动检查依赖/训练模型/拉起前后端）
...
1. **自建数据集（已开源）**——*LPBF 熔池监测与成形质量合成数据集*，已发布至 Hugging Face：

   🔗 **<https://huggingface.co/datasets/zoumingwu/lpbf-melt-pool-quality-dataset>**

   由 `scripts/generate_sample_data.py` 程序合成（随机种子固定，可复现），含 200 张熔池灰度图、
   200 条工艺参数记录、6 个打印任务的传感器时序。**该数据为合成数据，非真实企业/实验数据**。
   仓库内提交一份副本（`data/`）供离线演示与自动化测试。

2. **公开基准数据集**（真实工业数据，需自行下载，不随仓库提交）：
   - 主数据集：NIST AM-Bench 2022（增材制造基准，<https://www.nist.gov/ambench>），含 LPBF 熔池几何/温度与成形性能数据；
   - 辅助数据集：公开 LPBF 熔池监测数据集（Zenodo <https://zenodo.org> / 魔搭 <https://modelscope.cn> / 飞桨 AI Studio <https://aistudio.baidu.com> 检索）。
...
- [x] 数据集开源（Hugging Face：[lpbf-melt-pool-quality-dataset](https://huggingface.co/datasets/zoumingwu/lpbf-melt-pool-quality-dataset)）
- [x] 集成调试、设计说明书撰写（`设计说明书.md`）
- [ ] 演示视频与答辩 PPT
